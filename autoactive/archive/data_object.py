class Dataobject:
    ...
    def replaceNatives(self, **kwargs):
        ...
        for key in serializableObj["user"].keys():
            ...
            if (
                issubclass(
                    serializableObj["user"][key].__class__, self.__class__.__bases__
                )
                == True
            ):
                if serializableObj["user"][key].__class__.__name__ in [
                    "Table",
                    "Source",
                    "Video",
                ]:
                    ...
                    serializableObj["user"][key] = serializableObj["user"][
                        key
                    ].replaceNatives(**kwargs)
